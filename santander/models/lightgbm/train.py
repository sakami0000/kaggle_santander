...
def train_lgb(x_train, y_train, x_test, params, logger,
              n_splits=5, num_round=1000000, early_stop=4000, seed=2319):
    ...
    for fold, (train_idx, valid_idx) in enumerate(folds.split(x_train, y_train)):
        ...
        with step_timer(f'fold {fold + 1}', out=logger.info):
            ...
            dtrain = lgb.Dataset(train_fold_x, label=train_fold_y)
            ...
            feature_importances += clf.feature_importance(importance_type='gain') / n_splits
